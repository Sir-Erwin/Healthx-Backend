...
const server = http.createServer((req, res) => {
    ...
    if (req.method === 'DELETE' && req.url.startsWith('/Sapp/')) {
        ...
        pool.query('SELECT PATIENT_PID, FName, LName, Gender, Date, Time FROM APPOINTMENT', (error, results, fields) => {
            if (error) {
                res.writeHead(500, { 'Content-Type': 'application/json' });
                res.end(JSON.stringify({ error: 'Internal Server Error' }));
                return;
...
server.listen(PORT, () => {
    console.log(`Server running at http://localhost:${PORT}/Sapp`);
});