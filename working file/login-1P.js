...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/') {
    filePath = path.join(__dirname, '..', 'HealthX-Final', 'home', 'loginP.html');
...
function handleLoginRequest(req, res) {
  ...
  req.on('end', () => {
    ...
    // Check login credentials in the database
     db.query('SELECT Email, Passw FROM PATIENT WHERE Email = ? AND Passw = ?', [username, password], (err, results) => {
      if (err) {
